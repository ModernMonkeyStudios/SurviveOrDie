...
/** Scalability CVars */
static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshLODMorphEnabled(
	TEXT("r.Oceanology.WaterMesh.LODMorphEnabled"), 1,
	TEXT(
...
/** Debug CVars */
static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshShowWireframe(
	TEXT("r.Oceanology.WaterMesh.ShowWireframe"),
	0,
	TEXT("Forces wireframe rendering on for water"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshShowWireframeAtBaseHeight(
	TEXT("r.Oceanology.WaterMesh.ShowWireframeAtBaseHeight"),
	0,
	TEXT("When rendering in wireframe, show the mesh with no displacement"),
	ECVF_RenderThreadSafe
);

TAutoConsoleVariable<int32> CVarOceanologyWaterMeshEnableRendering(
	TEXT("r.Oceanology.WaterMesh.EnableRendering"),
	1,
	TEXT("Turn off all water rendering from within the scene proxy"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshShowLODLevels(
	TEXT("r.Oceanology.WaterMesh.ShowLODLevels"),
	0,
	TEXT("Shows the LOD levels as concentric squares around the observer position at height 0"),
	ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshShowTileBounds(
	TEXT("r.Oceanology.WaterMesh.ShowTileBounds"),
	0,
	TEXT(
...
static TAutoConsoleVariable<int32> CVarOceanologyWaterMeshPreAllocStagingInstanceMemory(
	TEXT("r.Oceanology.WaterMesh.PreAllocStagingInstanceMemory"),
	0,
	TEXT(
...
FOceanologyWaterMeshSceneProxy::FOceanologyWaterMeshSceneProxy(UOceanologyWaterMeshComponent* Component)
	  ...
	  , MaterialRelevance(Component->GetWaterMaterialRelevance(GetScene().GetFeatureLevel()))
{
	...
	if (Component->QuadTreeSettings.ForceCollapseDensityLevel > -1)
	{
		ForceCollapseDensityLevel = Component->QuadTreeSettings.ForceCollapseDensityLevel;
	}
	...
	{
		WaterVertexFactories.
			Add(new OceanologyWaterVertexFactoryType(GetScene().GetFeatureLevel(), NumQuads, LODScale));
	...
	const int32 TotalLeaFOceanologyNodes = WaterQuadTree.GetMaxLeafCount();
	WaterInstanceDataBuffers = new OceanologyWaterInstanceDataBuffersType(TotalLeaFOceanologyNodes);

	WaterMeshUserDataBuffers = new OceanologyWaterMeshUserDataBuffersType(WaterInstanceDataBuffers);
...
FOceanologyWaterMeshSceneProxy::~FOceanologyWaterMeshSceneProxy()
{
	for (OceanologyWaterVertexFactoryType* WaterFactory : WaterVertexFactories)
	{
...
FOceanologyWaterMeshSceneProxy::FOceanologyWaterLODParams FOceanologyWaterMeshSceneProxy::GetWaterLODParams(
	const FVector& Position) const
{
	...
	FOceanologyWaterLODParams WaterLODParams;
...
void FOceanologyWaterMeshSceneProxy::GetDynamicMeshElements(const TArray<const FSceneView*>& Views,
                                                            const FSceneViewFamily& ViewFamily, uint32 VisibilityMap,
                                                            FMeshElementCollector& Collector) const
{
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(Water);
	TRACE_CPUPROFILER_EVENT_SCOPE(FOceanologyWaterMeshSceneProxy::GetDynamicMeshElements);
	FRHICommandListBase& RHICmdList = FRHICommandListImmediate::Get();

	// The water render groups we have to render for this batch : 
	TArray<EOceanologyWaterMeshRenderGroupType, TInlineAllocator<OceanologyWaterVertexFactoryType::NumRenderGroups>>
		BatchRenderGroups;
	// By default, render all water tiles : 
	BatchRenderGroups.Add(EOceanologyWaterMeshRenderGroupType::RG_RenderWaterTiles);
	...
	if (bSelectionRenderEnabled && bHasSelectedInstances)
	{
		...
		BatchRenderGroups[0] = EOceanologyWaterMeshRenderGroupType::RG_RenderSelectedWaterTilesOnly;
		BatchRenderGroups.Add(EOceanologyWaterMeshRenderGroupType::RG_RenderUnselectedWaterTilesOnly);
	}
#endif // WITH_WATER_SELECTION_SUPPORT

	if (!HasWaterData() || !FOceanologyWaterUtils::IsWaterMeshRenderingEnabled(/*bIsRenderThread = */true))
	{
		return;
	}

	// Set up wireframe material (if needed)
	const bool bWireframe = AllowDebugViewmodes() && (ViewFamily.EngineShowFlags.Wireframe || CVarOceanologyWaterMeshShowWireframe
		.GetValueOnRenderThread() == 1);

	FColoredMaterialRenderProxy* WireframeMaterialInstance = nullptr;
	if (bWireframe && CVarOceanologyWaterMeshShowWireframeAtBaseHeight.GetValueOnRenderThread() == 1)
	{
	...
	TArray<FOceanologyWaterQuadTree::FOceanologyTraversalOutput, TInlineAllocator<4>> WaterInstanceDataPerView;
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if ((VisibilityMap & (1 << ViewIndex)) && (!bEncounteredISRView || View->IsPrimarySceneView()))
		{
			...
			FOceanologyWaterLODParams WaterLODParams = GetWaterLODParams(ObserverPosition);

#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			if (CVarOceanologyWaterMeshShowLODLevels.GetValueOnRenderThread())
			{
				for (int32 i = WaterLODParams.LowestLOD; i < WaterQuadTree.GetTreeDepth(); i++)
				{
					float LODDist = FOceanologyWaterQuadTree::GetLODDistance(i, LODScale);
					FVector Orig = FVector(FVector2D(ObserverPosition), WaterLODParams.WaterHeightForLOD);

					DrawCircle(Collector.GetPDI(ViewIndex), Orig, FVector::ForwardVector, FVector::RightVector,
					           GColorList.GetFColorByIndex(i + 1), LODDist, 64, 0);
				}
			}
#endif
			TRACE_CPUPROFILER_EVENT_SCOPE(QuadTreeTraversalPerView);

			FOceanologyWaterQuadTree::FOceanologyTraversalOutput& WaterInstanceData = WaterInstanceDataPerView.
				Emplace_GetRef();
			WaterInstanceData.BucketInstanceCounts.Empty(NumBuckets);
			WaterInstanceData.BucketInstanceCounts.AddZeroed(NumBuckets);
			if (!!CVarOceanologyWaterMeshPreAllocStagingInstanceMemory.GetValueOnRenderThread())
			{
				WaterInstanceData.StagingInstanceData.Empty(HistoricalMaxViewInstanceCount);
			}

			FOceanologyWaterQuadTree::FOceanologyTraversalDesc TraversalDesc;
			...
			TraversalDesc.bLODMorphingEnabled = !!CVarOceanologyWaterMeshLODMorphEnabled.GetValueOnRenderThread();
			TraversalDesc.TessellatedWaterMeshBounds = TessellatedWaterMeshBounds;

#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			//Debug
			TraversalDesc.DebugPDI = Collector.GetPDI(ViewIndex);
			TraversalDesc.DebugShowTile = CVarOceanologyWaterMeshShowTileBounds.GetValueOnRenderThread();
#endif
			WaterQuadTree.BuildWaterTileInstanceData(TraversalDesc, WaterInstanceData);

			HistoricalMaxViewInstanceCount =
				FMath::Max(HistoricalMaxViewInstanceCount, WaterInstanceData.InstanceCount);
		}
	}

	// Get number of total instances for all views
	int32 TotalInstanceCount = 0;
	for (const FOceanologyWaterQuadTree::FOceanologyTraversalOutput& WaterInstanceData : WaterInstanceDataPerView)
	{
		...
		if ((VisibilityMap & (1 << ViewIndex)) && (!bEncounteredISRView || Views[ViewIndex]->IsPrimarySceneView()))
		{
			...
			for (int32 MaterialIndex = 0; MaterialIndex < NumWaterMaterials; ++MaterialIndex)
			{
				...
				for (int32 DensityIndex = 0; DensityIndex < DensityCount; ++DensityIndex)
				{
					...
					const FMaterialRenderProxy* MaterialRenderProxy = (WireframeMaterialInstance != nullptr)
						                                                  ? WireframeMaterialInstance
						                                                  : WaterQuadTree.GetWaterMaterials()[
							                                                  MaterialIndex];
					...
					if (const FMaterial* BucketMaterial = MaterialRenderProxy->GetMaterialNoFallback(
					...
					bMaterialDrawn = true;
					for (EOceanologyWaterMeshRenderGroupType RenderGroup : BatchRenderGroups)
					{
						// Set up mesh batch
...
#if WITH_WATER_SELECTION_SUPPORT
						Mesh.bUseSelectionOutline = (RenderGroup ==
							EOceanologyWaterMeshRenderGroupType::RG_RenderSelectedWaterTilesOnly);
						Mesh.bUseWireframeSelectionColoring = (RenderGroup ==
							EOceanologyWaterMeshRenderGroupType::RG_RenderSelectedWaterTilesOnly);
#endif // WITH_WATER_SELECTION_SUPPORT
...

						{
							...
							BatchElement.NumPrimitives = WaterVertexFactories[DensityIndex]->IndexBuffer->
								GetIndexCount() / 3;
							BatchElement.MinVertexIndex = 0;
							BatchElement.MaxVertexIndex = WaterVertexFactories[DensityIndex]->VertexBuffer->
								GetVertexCount() - 1;
						...
						{
							INC_DWORD_STAT_BY(STAT_WaterVerticesDrawn,
							                  WaterVertexFactories[DensityIndex]->VertexBuffer->GetVertexCount() *
							                  InstanceCount);
			...
			{
				const FOceanologyWaterQuadTree::FOceanologyStagingInstanceData& Data = WaterInstanceData.
					StagingInstanceData[Idx];
				const int32 WriteIndex = WaterInstanceData.BucketInstanceCounts[Data.BucketIndex]++;

				for (int32 StreamIdx = 0; StreamIdx < OceanologyWaterInstanceDataBuffersType::NumBuffers; ++StreamIdx)
				{
					TArrayView<FVector4f> BufferMemory = WaterInstanceDataBuffers->GetBufferMemory(StreamIdx);
					for (int32 IdxMultipliedInstance = 0; IdxMultipliedInstance < InstanceFactor; ++
					     IdxMultipliedInstance)
					{
...
#if RHI_RAYTRACING
void FOceanologyWaterMeshSceneProxy::SetupRayTracingInstances(FRHICommandListBase& RHICmdList, int32 NumInstances,
                                                              uint32 DensityIndex)
{
	TArray<FOceanologyRayTracingWaterData>& WaterDataArray = RayTracingWaterData[DensityIndex];
	...
	if (WaterDataArray.Num() < NumInstances)
	{
		...
		static const FName DebugName("FOceanologyWaterMeshSceneProxy");
		...
		{
			FOceanologyRayTracingWaterData& WaterData = WaterDataArray.AddDefaulted_GetRef();
...
class TOceanologyWaterVertexFactoryUserDataWrapper : public FOneFrameResource
{
public:
	TOceanologyWaterMeshUserData<bWithWaterSelectionSupport> UserData;
};

void FOceanologyWaterMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingMaterialGatheringContext& Context,
                                                                   TArray<FRayTracingInstance>& OutRayTracingInstances)
{
	if (!HasWaterData() || !FOceanologyWaterUtils::IsWaterMeshRenderingEnabled(/*bIsRenderThread = */true) || !
		CVarRayTracingGeometryWater.GetValueOnRenderThread())
	{
		return;
	}
	...
	FOceanologyWaterQuadTree::FOceanologyTraversalOutput WaterInstanceData;
	WaterInstanceData.BucketInstanceCounts.Empty(NumBuckets);
	WaterInstanceData.BucketInstanceCounts.AddZeroed(NumBuckets);
	...
	{
		BucketOffsets[BucketIndex] = BucketOffsets[BucketIndex - 1] + WaterInstanceData.BucketInstanceCounts[BucketIndex
			- 1];
	}

	WaterInstanceData.StagingInstanceData.StableSort(
		[](const FOceanologyWaterQuadTree::FOceanologyStagingInstanceData& Lhs,
		   const FOceanologyWaterQuadTree::FOceanologyStagingInstanceData& Rhs)
		{
	...
	for (int32 DensityIndex = 0; DensityIndex < DensityCount; ++DensityIndex)
	{
		...
		for (int32 MaterialIndex = 0; MaterialIndex < NumWaterMaterials; ++MaterialIndex)
		{
			...
			{
				using FWaterVertexFactoryUserDataWrapperType = TOceanologyWaterVertexFactoryUserDataWrapper<
					WITH_WATER_SELECTION_SUPPORT>;
				FWaterVertexFactoryUserDataWrapperType& UserDataWrapper = Context.RayTracingMeshResourceCollector.
					AllocateOneFrameResource<FWaterVertexFactoryUserDataWrapperType>();

				const int32 InstanceDataIndex = BucketOffsets[BucketIndex] + InstanceIndex;
				const FOceanologyWaterQuadTree::FOceanologyStagingInstanceData& InstanceData = WaterInstanceData.
					StagingInstanceData[InstanceDataIndex];

				FOceanologyWaterVertexFactoryRaytracingParameters UniformBufferParams;
				...
				UserDataWrapper.UserData.InstanceDataBuffers = WaterMeshUserDataBuffers->GetUserData(
					EOceanologyWaterMeshRenderGroupType::RG_RenderWaterTiles)->InstanceDataBuffers;
				UserDataWrapper.UserData.RenderGroupType = EOceanologyWaterMeshRenderGroupType::RG_RenderWaterTiles;
				UserDataWrapper.UserData.WaterVertexFactoryRaytracingVFUniformBuffer =
					FOceanologyWaterVertexFactoryRaytracingParametersRef::CreateUniformBufferImmediate(
						UniformBufferParams, UniformBuffer_SingleFrame);

				BatchElement.UserData = (void*)&UserDataWrapper.UserData;

				FOceanologyRayTracingWaterData& WaterInstanceRayTracingData = RayTracingWaterData[DensityIndex][
					DensityInstanceIndex++];
...
FPrimitiveViewRelevance FOceanologyWaterMeshSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
#if WITH_WATER_SELECTION_SUPPORT
HHitProxy* FOceanologyWaterMeshSceneProxy::CreateHitProxies(UPrimitiveComponent* Component,
...
void FOceanologyWaterMeshSceneProxy::OnTessellatedWaterMeshBoundsChanged_GameThread(
	const FBox2D& InTessellatedWaterMeshBounds)
{
	check(IsInParallelGameThread() || IsInGameThread());

	FOceanologyWaterMeshSceneProxy* SceneProxy = this;
...
void FOceanologyWaterMeshSceneProxy::OnTessellatedWaterMeshBoundsChanged_RenderThread(
	const FBox2D& InTessellatedWaterMeshBounds)
{
